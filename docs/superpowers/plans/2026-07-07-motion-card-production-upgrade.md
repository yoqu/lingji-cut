...
6. timing 只按字幕句起点，尚未吸收停顿、重音、BGM 与剪辑节奏。
...
- [ ] **Task 3.1: `motion.bible` PromptKind 与默认模板**
  - 只输出 JSON。
  - 约束 carrier 分布，避免连续同型。
  - 验证：内置模板注册、变量完整。

- [ ] **Task 3.2: Bible 校验与降级**
...
- [ ] Motion 卡 Inspector 能展示生产质检报告、关键帧/缩略图、fallback 状态和 warning。
- [ ] Reviewer 能基于关键帧或明确降级路径输出可追踪审片结论。
- [ ] 一键分析能生成并保存 Motion Bible，单卡生成能消费它。
- [ ] 用户能在 UI 编辑 storyboard 并按当前分镜重雕。
- [ ] 新增至少 6 个专业 carrier，且每个 carrier 具备 kit 原语、fallback、校验与测试。
- [ ] TimingPlan 支持至少 SRT pause/accent，motion-kit 能基于 role 控制 anticipation/reveal/hold/resolve。
- [ ] 全部新增能力有单测或集成测试覆盖；关键渲染路径通过 smoke render。
