...
- 卡面数量、金额、百分比、比例、日期、排名和序号一律使用阿拉伯数字并保留原始单位与精度；中文口播只用于事实核对，不得照抄成中文数字。例：“十七万九千八百四十一辆”写为“179,841辆”，“百分之一百二十四点三”写为“124.3%”；不得换算或四舍五入，专有名词不改。
- 普通 Motion Card 必须输出 layout、elements、capacity 和每拍 lifecycle。elements 只有一个 focus(main)，最多再加一个 support(header) 与一个 asset(asset)；旧元素让位必须 collapse 或 exit，不能每拍只 enter 新内容。
...
- **载体不是你自由选的**：整片 bible 已为本段规划了 carrier，那是全片视角的多样性配额，默认照做。段落里有多个时点就画 trend、有多行多列就画 table、有多项数值对照就画 comparison——把这些改写成一个大数字或一句概念是塌陷，机器会打回。确实拿不出结构化数据时才改载体，并补 `carrierDeviation`。
- 可选的 `camera`（运镜）与 `annotate`（指示标注）是"讲清楚"的最高杠杆，也最容易滥用：只有当某一拍存在"观众必须看这一块"的时刻才写，各至多 2 项。camera 用 focus / push-in 把镜头带到正在讲的槽位，用 pull-out 收束回全局；annotate 用 circle / box / spotlight / arrow / underline / highlight 指出焦点，strike 专用于"这个说法是错的"。二者都是纯图形，不承载文字。没有明确焦点时整项省略。
...
- 若任务书包含「Agent 原子合成镜头锁定契约」，改用独立语义分镜：只输出 `claim`、`scene`、`focus:{beat,subject}`、`beats` 与 `media:[{assetId,slot,purpose,beats}]`。不得输出或依赖 `layout`、`elements`、`capacity`、固定版式 slot，也不要求 lifecycle；这些字段属于普通 Motion Card 模板，不适用于原子合成。素材池已在批准时冻结：不得新增 `assets`、替换绑定素材或引入未列出的素材。每个 `required` 素材都必须在 media 中声明其叙事作用和出现拍；`optional` 可基于叙事价值取舍。空间布局、裁切、蒙版和层级全部留给雕刻师自主决定。没有该契约时继续遵守上一条普通 Motion Card 资产规则。
